...
const RideStatusCard: React.FC<RideStatusCardProps> = ({ rideId, status, duration, onReturn, onReport,}) => {
    ...
    const handleOpenModal = (type: "return" | "fix") => {
        if (type === "return") {
            setModalContent({ title: "Trả xe", message: "Bạn có chắc chắn muốn trả xe không?", type: "parking" });
        } else {
            setModalContent({ title: "Báo xe hỏng", message: "Bạn có chắc chắn muốn báo hỏng xe không?", type: "error" });
        }
        setIsModalOpen(true);
    };
    ...
    return (
        <div className="fixed z-10 bottom-0 left-0 right-0 bg-white w-full h-[233px] rounded-t-2xl shadow-[0px_-4px_4px_0px_rgba(0,0,0,0.25)] flex flex-col p-8">
            ...
            <div className="flex justify-center py-4 gap-7">
                <div className="flex flex-col items-center cursor-pointer" onClick={() => handleOpenModal("return")}>
                    <img src={IconParking} alt="" className="w-[11px] pb-1 pt-0.5" />
                    <p className="text-[8px] font-semibold">Trả xe</p>
                </div>
                <div className="flex flex-col items-center cursor-pointer" onClick={() => handleOpenModal("fix")}>
